			...
			if(item != null) {
				for( FileItem file : context.getRequest().getUploadedFiles() )
				{
					if( "picture".equals(file.getFieldName()) && file.getSize() > 0 )
					{
						...
						{
							DynamicContentManager.remove(item.getPicture().substring("data/dynamicContent/".length()));
						}
					...
					if( "largepicture".equals(file.getFieldName()) && file.getSize() > 0 )
					{
						...
						{
							DynamicContentManager.remove(item.getLargePicture().substring("data/dynamicContent/".length()));
						}
