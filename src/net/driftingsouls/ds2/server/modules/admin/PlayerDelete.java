...
public class PlayerDelete implements AdminPlugin, Loggable {
	...
	public void output(AdminController controller, String page, int action) {
		...
		db.update("UPDATE skn SET userid=0 WHERE userid="+userid);
		...
		db.update("DELETE FROM sessions WHERE id="+userid);
		...
		db.update("DELETE FROM survey_voted WHERE user_id="+userid);
