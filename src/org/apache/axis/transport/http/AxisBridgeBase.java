...
public class AxisBridgeBase {
    ...
    public AxisBridgeBase(ServletConfig config, String configFile) {
    	this.config = config;
     ...
     */
    public static AxisServer getEngine(AxisBridgeBase servlet) throws AxisFault
    {
...
//              attach the AxisServer with the current Servlet
                engine.setName(servlet.getServletConfig().getServletName());
     ...
     */
    private static void storeEngine(AxisBridgeBase servlet, AxisServer engine) {
        ServletContext context = servlet.getServletContext();
        String axisServletName = servlet.getServletConfig().getServletName();
        if (engine == null) {
            ...
            if (server != null && servlet.getServletConfig().getServletName().equals(server.getName())) {
     ...
     */
    private static AxisServer retrieveEngine(AxisBridgeBase servlet) {
        Object contextObject = servlet.getServletContext().getAttribute(servlet.getServletConfig().getServletName() + ATTR_AXIS_ENGINE);
     ...
     * @return
     */
    protected static Map getEngineEnvironment(AxisBridgeBase servlet) {
        Map<String, Object> environment = new HashMap<String,Object>();

        String attdir= servlet.getServletConfig().getInitParameter(AxisEngine.ENV_ATTACHMENT_DIR);
        if (attdir != null)
        ...
        environment.put(EngineConfiguration.PROPERTY_NAME, new FileProvider(servlet.getConfigFile()));
                             ...
                             String dephault)
    {
        ...
        if (value == null)
            value = getServletConfig().getInitParameter(param);
