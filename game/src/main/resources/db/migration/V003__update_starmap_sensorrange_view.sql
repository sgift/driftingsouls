CREATE OR REPLACE VIEW friendly_scan_ranges AS SELECT s.star_system, s.x, s.y, ur.target_id, CONVERT(s.sensors*sm.sensorrange*0.01*((1.0 * (n.type IS NULL)) + (0.5* (n.type IS NOT NULL))), unsigned) sensor_range
