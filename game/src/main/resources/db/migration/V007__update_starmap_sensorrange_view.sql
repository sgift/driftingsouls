...
SELECT s.star_system,
       ...
       s.y,
       ur.target_id,
       CONVERT(MAX(s.sensors *
               IF(st.sensorrange >= COALESCE(sm.sensorrange, 0), st.sensorrange, sm.sensorrange) * 0.01 *
               ((1.0 * (n.type IS NULL)) + (0.5 * (n.type IS NOT NULL)))), unsigned) sensor_range
