...
public class VersteigerungResource extends Versteigerung {
	...
	public VersteigerungResource(User owner, Cargo res, long price) {
		...
		List<User> users = db.createQuery("from User", User.class).getResultList();
		
		for(User user: users)
		{
			//Abfrage, ob der user eine PM moechte
			if(user.getUserValue(WellKnownUserValue.GAMEPLAY_USER_AUKTION_PM)) {
				PM.send(niemand, user.getId(), "Neue Versteigerung eingestellt.", "Versteigert werden "+res.getResourceList().iterator().next().getCount1()+" " +res.getResourceList().iterator().next().getPlainName() +". Aktueller Preis: "+price+" RE", db);
			}
		}
	}
