...
public class TaskManager implements ApplicationContextAware {
	...
	public Task[] getTasksByTimeout( int timeout ) {
		List<Task> tasks = em.createQuery("from Task where timeout=:timeout order by time asc", Task.class)
	...
	public Task[] getTasksByData( Types type, String data1, String data2, String data3 ) {
		...
		TypedQuery<Task> q = em.createQuery(query, Task.class);
		q.setParameter("type", type.getTypeID());
		if( !data1.equals("*") ) {
	...
	public void reduceTimeout( int step ) {
		em.createQuery("update Task set timeout=timeout-:timeout where timeout>:timeout2")
			.setParameter("timeout", step)
			.setParameter("timeout2", step-1)
			.executeUpdate();

		if( step > 1 ) {
			em.createQuery("update Task set timeout=0 where timeout<=:timeout")
				.setParameter("timeout", step-1)
				.executeUpdate();
		}
	}
