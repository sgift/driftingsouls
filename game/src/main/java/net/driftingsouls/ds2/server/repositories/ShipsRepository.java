            ...
            var select =
                    db.select(SHIPS.ID,
                            ...
                            .where(SHIPS.STAR_SYSTEM.eq(location.getSystem())
                                    .and(SHIPS.X.eq(location.getX()))
                                    .and(SHIPS.Y.eq(location.getY())))
                            .and((SHIPS_MODULES.SIZE.isNotNull().and(SHIPS_MODULES.SIZE.greaterOrEqual(minSize)).or(SHIPS_MODULES.SIZE.isNull().and(SHIP_TYPES.SIZE.greaterOrEqual(minSize)))).and(SHIP_TYPES.CLASS.ne(ShipClasses.FELSBROCKEN.ordinal())))
                    ;
